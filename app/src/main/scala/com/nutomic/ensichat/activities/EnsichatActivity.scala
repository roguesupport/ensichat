...
import com.nutomic.ensichat.service.{EnsichatServiceBinder, EnsichatService}

/**
 * Connects to [[EnsichatService]] and provides access to it.
 */
class EnsichatActivity extends AppCompatActivity with ServiceConnection {

  private var chatService: Option[EnsichatService] = None
  ...
  override def onCreate(savedInstanceState: Bundle): Unit = {
    ...
    startService(new Intent(this, classOf[EnsichatService]))
    bindService(new Intent(this, classOf[EnsichatService]), this, Context.BIND_AUTO_CREATE)
  }
  ...
  override def onServiceConnected(componentName: ComponentName, iBinder: IBinder): Unit = {
    val binder = iBinder.asInstanceOf[EnsichatServiceBinder]
  ...
  /**
   * Calls l as soon as [[EnsichatService]] first becomes available.
   */
